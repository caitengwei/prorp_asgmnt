...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
    ...
    def sanitized: Observable[String] = {
      obs.map(term => term.replaceAll(" ", "_"))
    }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = Observable.create(
      ob => {
        obs.subscribe (
          t => ob.onNext(Success(t)),
          error => {
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      obs.map(op => requestMethod(op).recovered.onErrorReturn(t => Failure(t))).concat
    }
